...
//				$customer_name = $customer_details->name;
				...
				for($i=1; $i<=500; $i++){
					if( $this->input->post($quantity.$i) && $this->input->post($product.$i) && $this->input->post($unit_price.$i) ) {
						...
						if(DISCOUNT_METHOD == 1 && DISCOUNT_OPTION == 2) {
							...
							if($dsType == 1) { $discount[] = ($ds+$prm)."%"; } else { $discount[] = $ds+$prm;  }	

							if($prmType == 2) { $getBuy[] = 0; $promo[]=''; } else { 
							...
							if($prmType == 2) { $getBuy[] = 0; $promo[]=''; } else { $promo[] = $product_details->promo_title;
                              $getBuy[] = $this->input->post($quantity.$i);
							}	
   ...
   function scan_product()
   {
	   ...
	   if($prodd = $this->pos_model->getProductByCode($code)) {
		    ...
		    if($this->input->get('customer')) { $customer_id = $this->input->get('customer'); }
   ...
   function getOrderdetails()
   {
	   ...
	   if($prodd = $this->pos_model->getAllInvoiceItems($order->id)) {
...
th, td {
	...
	<table border="0" cellpadding="0" cellspacing="0" class="protable">
	   ...
	   foreach($prodd as $value){
		   ?>
	<tr>
	<td><input type="hidden" name="return_price[]" value="<?php echo $value->unit_price;  ?>"  /><input type="hidden" name="return_product_id[]" value="<?php echo $value->product_id;  ?>"  /><input type="hidden" id="order_items<?php echo $value->product_id;  ?>" name="orderitems_id[]" value="<?php echo $value->id;  ?>"  /><?php echo $value->product_name;  ?></td><td><?php echo $value->unit_price;  ?></td><td><?php echo $value->quantity;  ?></td><td><input type="text" id="ret_<?php echo $value->product_id;  ?>" name="returnproduct[]" style="width:50px;" /></td>
